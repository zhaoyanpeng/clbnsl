 ...
 * 
 */
package edu.shanghaitech.ai.ml.clbnsl.weka.classifiers.bayes.net;
...
public class ParentSet {
	...
	public int getFreshCardinalityOfParents(Instances _Instances) {
		...
		for (int iParent = 0; iParent < m_nNrOfParents; iParent++) {
			m_nCardinalityOfParents *= _Instances
					.attribute(m_nParents[iParent]).numValues();
		}
		return m_nCardinalityOfParents;
	}
	 ...
	 *            parent to add
	 * @param iParent
	 *            location to add parent in parent set
	 * @param _Instances
	 *            used for updating the internals
	 */
	 ...
	 * 
	 * @param nParent
	 *            node number of the parent to delete
	 * @param _Instances
	 *            data set
