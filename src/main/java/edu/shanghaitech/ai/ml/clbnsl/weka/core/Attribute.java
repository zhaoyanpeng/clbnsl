...
public class Attribute implements Copyable, Serializable {
	...
	public Attribute(String attributeName, List<String> attributeValues,
		...
		m_Name = attributeName;
		m_AttributeInfo = new NominalAttributeInfo(attributeValues,
				attributeName);
		if (attributeValues == null) {
			m_Type = STRING;

			// Make sure there is at least one value so that string attribute
			// values are always represented when output as part of a sparse
			// instance.
		...
		if (isNominal() || isString()) {
			...
			return new Enumeration<Object>() {
				...
				public Object nextElement() {
					...
					/*
					 * if (oo instanceof SerializedObject) { return
					 * ((SerializedObject) oo).getObject(); } else {
					 */
					return oo;
					// }
				}
	...
	public final String equalsMsg(Object other) {
		...
		if (isNominal() && att.isNominal()) {
			if (((NominalAttributeInfo) m_AttributeInfo).m_Values.size() != ((NominalAttributeInfo) att.m_AttributeInfo).m_Values
					.size()) {
				return "Different number of labels: "
						+ ((NominalAttributeInfo) m_AttributeInfo).m_Values
								.size()
						+ " != "
						+ ((NominalAttributeInfo) att.m_AttributeInfo).m_Values
								.size();
			}

			for (int i = 0; i < ((NominalAttributeInfo) m_AttributeInfo).m_Values
					.size(); i++) {
		...
		if ((type() != att.type())) {
			return "Types differ: " + typeToString(this) + " != "
					+ typeToString(att);
		}
	...
	public final int indexOfValue(String value) {
		...
		// @debug
		// System.out.println("indexOfValue: " + value);
		// System.out.println("isNominal: " + isNominal() + "; isString: " +
		// isString());
		...
		if (value.length() > STRING_COMPRESS_THRESHOLD) {
			/*
			 * try { store = new SerializedObject(value, true); } catch
			 * (Exception ex) {
			 * System.err.println("Couldn't compress string attribute value -" +
			 * " searching uncompressed."); }
			 */
		}
		Integer val = ((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
				.get(store);

		// @debug
		// sb le, bug tiao dao zhe li wo ye shi zui le.
	...
	public final String toString() {
		...
		case RELATIONAL:
			...
			Enumeration<Attribute> enm = ((RelationalAttributeInfo) m_AttributeInfo).m_Header
					.enumerateAttributes();
			while (enm.hasMoreElements()) {
				text.append(enm.nextElement()).append("\n");
			}
			text.append(ARFF_END_SUBRELATION).append(" ")
					.append(Utils.quote(m_Name));
			break;
		default:
			text.append("UNKNOWN");
			break;
	...
	public final/* @ non_null pure @ */String value(int valIndex) {
		...
		if (!isNominal() && !isString()) {
			...
			Object val = ((NominalAttributeInfo) m_AttributeInfo).m_Values
					.get(valIndex);

			// If we're storing strings compressed, uncompress it.
			/*
			 * if (val instanceof SerializedObject) { val = ((SerializedObject)
			 * val).getObject(); }
			 */
			return (String) val;
		}
	...
	public final/* @ non_null pure @ */Instances relation(int valIndex) {
		...
		if (!isRelationValued()) {
			...
			return (Instances) ((RelationalAttributeInfo) m_AttributeInfo).m_Values
					.get(valIndex);
		}
	}
	...
	public int addStringValue(String value) {
		...
		Integer index = ((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
				.get(store);
		if (index != null) {
			return index.intValue();
		} else {
			int intIndex = ((NominalAttributeInfo) m_AttributeInfo).m_Values
					.size();
	...
	public int addStringValue(Attribute src, int index) {
		...
		Object store = ((NominalAttributeInfo) src.m_AttributeInfo).m_Values
				.get(index);
		Integer oldIndex = ((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
				.get(store);
		if (oldIndex != null) {
	...
	public int addRelation(Instances value) {
		...
		if (!((RelationalAttributeInfo) m_AttributeInfo).m_Header
				.equalHeaders(value)) {
			throw new IllegalArgumentException(
					"Incompatible value for "
							+ "relation-valued attribute.\n"
							+ ((RelationalAttributeInfo) m_AttributeInfo).m_Header
									.equalHeadersMsg(value));
		}
		Integer index = ((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
				.get(value);
		if (index != null) {
			...
			((NominalAttributeInfo) m_AttributeInfo).m_Hashtable.put(value,
					new Integer(intIndex));
			return intIndex;
		}
	}
	...
	final void addValue(String value) {
		...
		((NominalAttributeInfo) m_AttributeInfo).m_Hashtable = Utils
				.cast(((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
						.clone());
		forceAddValue(value);
	}
	...
	final void delete(int index) {
		...
		if (!isNominal() && !isString() && !isRelationValued()) {
			...
			((NominalAttributeInfo) m_AttributeInfo).m_Values = Utils
					.cast(((NominalAttributeInfo) m_AttributeInfo).m_Values
							.clone());
			...
			if (!isRelationValued()) {
				Hashtable<Object, Integer> hash = new Hashtable<Object, Integer>(
						((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
								.size());
				Enumeration<Object> enu = ((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
						.keys();
				while (enu.hasMoreElements()) {
					Object string = enu.nextElement();
					Integer valIndexObject = ((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
							.get(string);
	...
	final void forceAddValue(String value) {
		...
		((NominalAttributeInfo) m_AttributeInfo).m_Values.add(store);
		((NominalAttributeInfo) m_AttributeInfo).m_Hashtable.put(
				store,
				new Integer(((NominalAttributeInfo) m_AttributeInfo).m_Values
						.size() - 1));
	}
	...
	final void setValue(int index, String string) {
		...
		case STRING:
			...
			((NominalAttributeInfo) m_AttributeInfo).m_Hashtable
					.remove(((NominalAttributeInfo) m_AttributeInfo).m_Values
							.get(index));
	...
	final void setValue(int index, Instances data) {
		...
		if (isRelationValued()) {
			if (!data
					.equalHeaders(((RelationalAttributeInfo) m_AttributeInfo).m_Header)) {
				throw new IllegalArgumentException(
