...
public final class Utils {
	...
	public static Object forName(Class<?> classType, String className,
		...
		if (System.getProperty("weka.test.maventest", "").equalsIgnoreCase(
				"true")) {
	...
	protected static Object forNameNoSchemeMatch(Class classType,
		...
		if (!classType.isAssignableFrom(c)) {
			throw new Exception(classType.getName()
					+ " is not assignable from " + className);
		}
		Object o = c.newInstance();
	...
	public static/* @pure@ */int[] stableSort(double[] array) {
		...
		if (array.length > 1) {
			...
			while (i < index.length) {
				numEqual = 1;
				for (int j = i + 1; ((j < index.length) && Utils.eq(
	 ...
	 * pivot.
	 */
	private static int sortLeftRightAndCenter(double[] array, int[] index,
			int l, int r) {
	...
	private static void quickSort(/* @non_null@ */double[] array, /* @non_null@ */
		...
		default:
			...
			// Establish pivot
			int pivotLocation = sortLeftRightAndCenter(array, index, left,
					right);

			// Move pivot to the right, partition, and restore pivot
			swap(index, pivotLocation, right - 1);
			int center = partition(array, index, left, right,
					array[index[right - 1]]);
	...
	private static int select(/* @non_null@ */double[] array, /* @non_null@ */
		...
		default:
			...
			if ((center - left + 1) >= k) {
				...
				return select(array, index, center + 1, right, k
						- (center - left + 1));
			}
		}
	}
	...
	public static String getGlobalInfo(Object object, boolean addCapabilities) {
		...
		if (addCapabilities) {
			...
			// @debug
			/*
			 * if (object instanceof CapabilitiesHandler) { if
			 * (!result.toString().endsWith("<br><br>")) {
			 * result.append("<br>"); } String caps =
			 * PropertySheetPanel.addCapabilities(
			 * "<font color=red>CAPABILITIES</font>", ((CapabilitiesHandler)
			 * object).getCapabilities()); caps = Utils.lineWrap(caps,
			 * lineWidth).replace("\n", "<br>"); result.append(caps); }
			 * 
			 ...
			 * .getMultiInstanceCapabilities()); caps = Utils.lineWrap(caps,
			 * lineWidth).replace("\n", "<br>"); result.append(caps); }
			 */
	...
	public static void main(String[] ops) {
		...
		try {
			...
			System.out
					.println("Array-Dimensions of 'new int[][]{{1,2,3},{4,5,6}}': "
							+ Utils.getArrayDimensions(new int[][] {
									{ 1, 2, 3 }, { 4, 5, 6 } }));
